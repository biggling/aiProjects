...
case "$ACTION" in
  status)
    echo "=== PROJECT STATUS ==="
    for proj in trade-auto pod shopee-affiliate amazon-kdp steam-game android-app polymarket; do
      cf="$PROJECT_ROOT/projects/$proj/continue.md"
      if [[ -f "$cf" ]]; then
  ...
  trade-auto|pod|shopee-affiliate|amazon-kdp|steam-game|android-app|polymarket|all)
    echo "Running $ACTION..."
